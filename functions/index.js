...
exports.fetchResultsScheduled = functions.pubsub
    ...
    try {
      ...
      for (const apim of finished) {
        ...
        for (const d of snap.docs) {
          ...
          const homeMatch = apiHome.toLowerCase().includes(local.h.toLowerCase().slice(0, 5)) ||
            local.h.toLowerCase().includes(apiHome.toLowerCase().slice(0, 5));
          const awayMatch = apiAway.toLowerCase().includes(local.a.toLowerCase().slice(0, 5)) ||
      ...
      const stageMap = {
        ...
        'THIRD_PLACE': '3ème Place', 'FINAL': 'Finale'
      };
      const knockouts = (data.matches || []).filter(m =>
        stageMap[m.stage] && m.homeTeam?.name && m.awayTeam?.name
      );

      const batch = db.batch();
      const existSnap = await db.collection('matches').get();
      const existExtIds = new Set(existSnap.docs.map(d => d.data().externalId).filter(Boolean));
      ...
      console.log(`✅ Sync completed: ${finished.length} finished, ${knockouts.length} knockouts`);
    } catch (error) {
